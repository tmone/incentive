    ...
    var onAfterExec_Tern = function(e, commandManager) {
        if (e.command.name === "insertstring" && (e.args === "." || e.args ==='"')) {
            if (e.editor.ternServer && e.editor.ternServer.enabledAtCurrentLocation(e.editor)) {
                ...
                var isOn = false;
                var pre = e.editor.session.getTokenAt(pos.row,pos.column - 1);
                if(pre.type==="paren.lparen"){
                    var ch = e.editor.session.getTokenAt(pos.row,pre.start);
                    if(ch.value==="on"){
                        isOn = true;
                    }
                }
                if (tok) {
                    if (tok.type.toString().indexOf('comment') === -1 ) {
                        try {
