            ...
            $find.dxTreeList({
                    ...
                    },
                {
                    ...
                    cellTemplate: function (container, options) {
                        var dat = [
                            $("<div>").append($("<i class='fa fa-th-list'>"), $("<span>").text(" parent Menu")),
                            $("<div>").append($("<i class='fa fa-file-photo-o'>"), $("<span>").text(" System Page")),
                            $("<div>").append($("<i class='fa fa-link'>"), $("<span>").text(" Url")),
                        ]; 
                        
                        $(dat[options.value]).appendTo(container);
                    },
                    editCellTemplate: function (cellElement, cellInfo) {
                        var div = document.createElement("div");
                        cellElement.get(0).appendChild(div);
                        $(div).dxSelectBox({
                            dataSource: [{id:0, name:"Parent Menu"},{id:1,name:"System Page"}, {id:2, name:"Url"}],
                            displayExpr: "name",
                            valueExpr: "id",
                            value: cellInfo.value || 0,
                            onValueChanged: function (e) {
                                cellInfo.setValue(e.value);
                                if (e.value == 1) {
                                    that.Component.columnOption("pageid", "formItem.visible", true);
                                    that.Component.columnOption("target", "formItem.visible", false);
                                } else if (e.value == 2) {
                                    that.Component.columnOption("pageid", "formItem.visible", false);
                                    that.Component.columnOption("target", "formItem.visible", true);
                                } else {
                                    that.Component.columnOption("pageid", "formItem.visible", false);
                                    that.Component.columnOption("target", "formItem.visible", false);
                                }
                ...
                },
                {
                    dataField: "target",  
                    cellTemplate: function (container, options) {
                        var dat = $("<a href='" + options.value + "' target='_blank'>").text(options.value);                        

                        dat.appendTo(container);
                    },                    
                    formItem: {
                        visible:false,
                    }
                ...
                },
                {
                    dataField: "icon",
                    cellTemplate: function (cellElement, cellInfo) {
                        cellElement.append("<i class='" + cellInfo.value + "' style='padding-right:10px'>").append(cellInfo.value);
                    },
                    visible: false,
                    editCellTemplate: function (cellElement, cellInfo) {
                        var div = document.createElement("div");
                        ...
                        $(div).dxSelectBox({
                            ...
                            itemTemplate: function (data) {
                                return "<i class='" + data.fullname + " pull-right'></i>" + data.name;
                            },
                            searchEnabled: true,
