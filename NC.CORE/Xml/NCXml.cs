...
namespace NC.CORE.Xml
{
    public class NCXml
    {
        public dynamic XmlToDynamic(string path)
        {
            if (File.Exists(path))
            {
                string readText = File.ReadAllText(path);
                string str_JON = this.XmlToJSON(readText);
                dynamic json = JsonConvert.DeserializeObject(str_JON);
                //NCLogger.Debug(str_JON);
                return json;
            }
            return null;
