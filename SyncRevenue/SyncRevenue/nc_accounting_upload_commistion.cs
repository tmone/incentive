...
namespace SyncRevenue
{
    public class nc_accounting_upload_commistion
    {
        ...
        {
            return _conn.Query<nc_accounting_upload_commistion>("select * from nc_accounting_upload_commistion where id = @id", new { id }).FirstOrDefault();
        }
        public nc_accounting_upload_commistion findWhere(string where = "1=0")
        {
            return _conn.Query<nc_accounting_upload_commistion>("select * from nc_accounting_upload_commistion where " + where).FirstOrDefault();
        ...
        public void save(bool overider = true)
        {
            ...
            _conn.Execute(@"UPDATE [nc_accounting_upload_commistion]
        ...
        public void remove()
        {
            _conn.Execute(@"DELETE FROM [nc_accounting_upload_commistion] WHERE id = @id", this);
        }

        private int addNew()
        {
            return _conn.Query<int>(@"INSERT INTO [nc_accounting_upload_commistion](_active,_deleted,_createdate) VALUES(1,0,getdate());SELECT CAST(SCOPE_IDENTITY() as int)").FirstOrDefault();
