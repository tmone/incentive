...
namespace SyncRevenue
{
    public class nc_accounting_temp_invoice
    {
        ...
        {
            return _conn.Query<nc_accounting_temp_invoice>("select * from nc_accounting_temp_invoice where id = @id", new { id }).FirstOrDefault();
        }
        public nc_accounting_temp_invoice findWhere(string where = "1=0")
        {
            return _conn.Query<nc_accounting_temp_invoice>("select * from nc_accounting_temp_invoice where " + where).FirstOrDefault();
        ...
        public void save(bool overider = true)
        {
            ...
            _conn.Execute(@"UPDATE [nc_accounting_temp_invoice]
                 SET [CN] = @CN
                    ,[SO_HD] = @SO_HD
        ...
        public void remove()
        {
            _conn.Execute(@"DELETE FROM [nc_accounting_temp_invoice] WHERE id = @id", this);
        }

        private int addNew()
        {
            return _conn.Query<int>(@"INSERT INTO [nc_accounting_temp_invoice](SO_HD) VALUES(null);SELECT CAST(SCOPE_IDENTITY() as int)").FirstOrDefault();
