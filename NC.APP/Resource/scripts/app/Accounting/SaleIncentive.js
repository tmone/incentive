                ...
                dataSource: {
                        ...
                        load: function (loadOptions) {
                            ...
                            function loadElastic(lastID){                                
                                $.ajax({
                                    ...
                                    //url:"http://10.10.10.87:9200/revenue/_search?size=10000&from="+lastID,
                                    dataType: "json",
                                    //data: args,
                                    // success: function (result) {
                                    //     if(result.hits && result.hits.hits.length>0){
                                    //         RESULT = RESULT.concat(result.hits.hits.map(x=>x._source));
                                    //         if(RESULT.length< result.hits.total){
                                    //             lastID = RESULT.length;
                                    //             DevExpress.ui.notify("Load "+lastID+ " / "+result.hits.total,"warning",1000);
                                    //             loadElastic(lastID);
                                    //         }else{
                    ...
                    fields: [
                        ...
                        },
                        {
                            ...
                            caption:"Incentive Special Rate",
                            dataType: "number",
                            format: "###,##0",
                            summaryType: "sum",
                            area: "data",
                    ...
                    if(va && va.length>0){
                        for(var i = 0; i<va.length; i++){
                            ...
                            if(va[i].name && va[i].name.indexOf("Ar") > -1){
                                ...
                                if(va.filter(x=>x.title=='AR').length==0){
                                    va[i].title = "AR";
                                }else{
                                    va[i].title = null;
                                }
                            ...
                            if(va[i].name && va[i].name.indexOf("Incentive") > -1){
                                va[i].label.visible = false;
                                if(va.filter(x=>x.title=='AR').length==0){
                                    va[i].title = "AR";
                                }else{
                            ...
                            if(va[i].name && va[i].name.indexOf("Bonus") > -1){
                                va[i].label.visible = false;
                                if(va.filter(x=>x.title=='AR').length==0){
                                    va[i].title = "AR";
                                }else{
                            ...
                            if(va[i].name && va[i].name.indexOf("Target") > -1){
                                va[i].label.visible = false;
                                if(va.filter(x=>x.title=='AR').length==0){
                                    va[i].title = "AR";
                                }else{
...
if (typeof module !== 'undefined' && typeof module.exports !== 'undefined')
    ...
    window.SaleIncentive = SaleIncentive;