...
namespace NC.API.App.Accounting.Models
{
    public class nc_accounting_temp_revenue
    {
        ...
        {
            return _context._db._conn.Query<nc_accounting_temp_revenue>("select * from nc_accounting_temp_revenue where id = @id", new { id }).FirstOrDefault();
        }
        public nc_accounting_temp_revenue findWhere(string where = "1=0")
        {
            return _context._db._conn.Query<nc_accounting_temp_revenue>("select * from nc_accounting_temp_revenue where " + where).FirstOrDefault();
        ...
        public void save()
        {
            ...
            _context._db._conn.Execute(@"UPDATE [nc_accounting_temp_revenue]
             SET [MA_KH] = @MA_KH
                ,[IN_MONTH] = @IN_MONTH
        ...
        public void remove()
        {
            _context._db._conn.Execute(@"DELETE FROM [nc_accounting_temp_revenue] WHERE id = @id", this);
        }

        private int addNew()
        {
            return _context._db._conn.Query<int>(@"INSERT INTO [nc_accounting_temp_revenue](MA_KH) VALUES(null);SELECT CAST(SCOPE_IDENTITY() as int)").FirstOrDefault();
